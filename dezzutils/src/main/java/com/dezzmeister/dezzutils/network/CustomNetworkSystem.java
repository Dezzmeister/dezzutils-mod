...
public class CustomNetworkSystem extends NetworkSystem {
	...
	public static final LazyValue<NioEventLoopGroup> SERVER_NIO_EVENTLOOP = new LazyValue<>(() -> {
		return new NioEventLoopGroup(0,
				(new ThreadFactoryBuilder()).setNameFormat("Netty Server IO #%d").setDaemon(true)
						.setThreadFactory(net.minecraftforge.fml.common.thread.SidedThreadGroups.SERVER).build());
	});
	...
	public CustomNetworkSystem(MinecraftServer server) {
		super(server);
		this.server = server;
		this.isAlive = true;
	}
		...
		synchronized (this.endpoints) {
			...
			this.endpoints.add((new ServerBootstrap()).channel(oclass).childHandler(new ChannelInitializer<Channel>() {
				protected void initChannel(Channel p_initChannel_1_) throws Exception {
					...
					p_initChannel_1_.pipeline().addLast("timeout", new ReadTimeoutHandler(READ_TIMEOUT))
							.addLast("legacy_query", new LegacyPingHandler(CustomNetworkSystem.this))
					...
					networkmanager.setNetHandler(
							new CustomServerHandshakeNetHandler(CustomNetworkSystem.this.server, networkmanager));
				}
			...
			channelfuture = (new ServerBootstrap()).channel(LocalServerChannel.class)
					.childHandler(new ChannelInitializer<Channel>() {
						protected void initChannel(Channel p_initChannel_1_) throws Exception {
							NetworkManager networkmanager = new NetworkManager(PacketDirection.SERVERBOUND);
							networkmanager.setNetHandler(
									new ClientHandshakeNetHandler(CustomNetworkSystem.this.server, networkmanager));
							CustomNetworkSystem.this.networkManagers.add(networkmanager);
							p_initChannel_1_.pipeline().addLast("packet_handler", networkmanager);
						}
					}).group(SERVER_NIO_EVENTLOOP.getValue()).localAddress(LocalAddress.ANY).bind()
					.syncUninterruptibly();
						...
						try {
							...
							if (networkmanager.isLocalChannel()) {
								throw new ReportedException(
										CrashReport.makeCrashReport(exception, "Ticking memory connection"));
							}
