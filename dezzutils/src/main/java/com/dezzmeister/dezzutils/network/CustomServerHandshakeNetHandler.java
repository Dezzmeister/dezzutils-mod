...
public class CustomServerHandshakeNetHandler extends ServerHandshakeNetHandler implements IHandshakeNetHandler {
	private static final ITextComponent field_241169_a_ = new StringTextComponent("Ignoring status request");
	private final MinecraftServer server;
	private final NetworkManager networkManager;

	public CustomServerHandshakeNetHandler(MinecraftServer serverIn, NetworkManager netManager) {
	...
	public void processHandshake(CHandshakePacket packetIn) {
		if (!net.minecraftforge.fml.server.ServerLifecycleHooks.handleServerLogin(packetIn, this.networkManager))
			return;
		...
		case LOGIN:
			...
			if (packetIn.getProtocolVersion() > SharedConstants.getVersion().getProtocolVersion()) {
				ITextComponent itextcomponent = new TranslationTextComponent("multiplayer.disconnect.outdated_server",
				...
				ITextComponent itextcomponent1 = new TranslationTextComponent("multiplayer.disconnect.outdated_client",
						SharedConstants.getVersion().getName());
		...
		case STATUS:
			if (this.server.func_230541_aj_()) {
				...
				this.networkManager.setNetHandler(new CustomServerStatusNetHandler(this.server, this.networkManager));
			} else {
