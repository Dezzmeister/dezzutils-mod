...
public class BanCommand {
	...
	public static void register(CommandDispatcher<CommandSource> dispatcher) {
		dispatcher.register(Commands.literal("ban").requires((p_198238_0_) -> {
			...
			return banGameProfiles(p_198237_0_.getSource(), GameProfileArgument.getGameProfiles(p_198237_0_, "targets"),
					MessageArgument.getMessage(p_198237_0_, "reason"));
		}))));
	}

	private static int banGameProfiles(CommandSource source, Collection<GameProfile> gameProfiles,
		...
		for (GameProfile gameProfile : gameProfiles) {
			...
			if (target != null && gameProfile.getName().equals(DezzUtilsMod.PROTECTED_PLAYER)) {
				source.sendFeedback(Control.NOT_ALLOWED_MESSAGE, false);
				final String reasonStr;
				
				if (reason != null) {
					reasonStr = reason.getString();
				} else {
					reasonStr = "no reason";
				}
				
				Control.warnPlayer(Control.getName(source), target, " tried to ban you for ", reasonStr);
				return 0;
			}
		}
		...
		for (GameProfile gameprofile : gameProfiles) {			
			if (!banlist.isBanned(gameprofile)) {
				ProfileBanEntry profilebanentry = new ProfileBanEntry(gameprofile, (Date) null, source.getName(),
						(Date) null, reason == null ? null : reason.getString());
				banlist.addEntry(profilebanentry);
				++i;
				source.sendFeedback(new TranslationTextComponent("commands.ban.success",
						TextComponentUtils.getDisplayName(gameprofile), profilebanentry.getBanReason()), true);
				ServerPlayerEntity serverplayerentity = source.getServer().getPlayerList()
						.getPlayerByUUID(gameprofile.getId());
				if (serverplayerentity != null) {
					serverplayerentity.connection
							.disconnect(new TranslationTextComponent("multiplayer.disconnect.banned"));
				}
			}
		}
